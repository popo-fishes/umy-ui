<template>
  <div>
    <h3>手动触发编辑表格(表格数据少，所以用data方式赋值表格数据。这是个示例)</h3>
    <p>
    ...
    <p>
      4.设置 keep-source 开启保持原始值状态，对于某些需要局部保存的场景，可以在数据保存完成后调用 reloadRow 方法加载行数据并恢复到初始状态
    </p>
    <ux-grid border
             show-overflow
             ref="plxTable"
...
<script>
  import md2 from '../../components/u-table-component/editTypeTbalePage/manualTrigger.md'
  export default {
    computed: {
      md () {
        return md2
