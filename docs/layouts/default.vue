<template>
  <div class="layoutIndexBox card-box" ref="cardBox">
    <div class="navbar width100">
      ...
      <div class="nav-pl">
        <div class="moshi" v-if="releaseShow()" style="color: red">
          umy-ui发布新版本啦！！！
        </div>
        <!--        <div class="moshi"><el-checkbox v-model="checked">设置播放器模式: mini</el-checkbox></div>-->
        <div class="search-box">
          <el-input placeholder="搜索文档关键字" v-model="docKeyWord" clearable>
          </el-input>
        </div>
        <div class="linkList">
          <div
            class="nav-item user-select"
            v-for="(item, index) in navData"
            :class="{
              active:
                activeUrl === item.url ||
                (componentActive && item.name === '组件')
            }"
            :key="index"
          ...
          <div class="nav-item">
            <a href="https://github.com/popo-fishes/umy-ui" target="_blank"
              >GitHub</a
            >
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    releaseShow() {
      let threeDaysLater = moment(this.timeText)
        .add(3, "days")
        .format("YYYY-MM-DD");
